...
from models import get_db, init_db, OverheadVendor, OverheadExpense
...
def generate_overhead_payment_pdf(report_data, month_name, total_amount, category_filter, payment_terms_filter, responsible_filter):
    """Generate PDF payment summary matching COGS style"""
    buffer = io.BytesIO()
    ...
    elements.append(Paragraph("CHELA<br/>Overhead Payment Summary", title_style))
...
def generate_overhead_pdf(expense_data, filters_text, total_amount, count):
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4)
    elements = []
    styles = getSampleStyleSheet()
    ...
    elements.append(Paragraph("CHELA<br/>Overhead Expenses Report", title_style))
...
def show_overview(db):
    ...
    expenses_this_month = db.query(OverheadExpense).filter(
        OverheadExpense.month == current_month,
        OverheadExpense.year == current_year
    ).all()

    total = sum(e.amount for e in expenses_this_month)
    vendor_count = db.query(OverheadVendor).filter(OverheadVendor.is_active == True).count()
...
def show_vendors(db):
    ...
    with tab1:
        vendors = db.query(OverheadVendor).filter(OverheadVendor.is_active == True).order_by(
            OverheadVendor.category, OverheadVendor.name
        ).all()
        ...
        with st.form("add_overhead_vendor", clear_on_submit=True):
            ...
            if st.form_submit_button("Add Vendor", type="primary", use_container_width=True):
                if not vendor_name:
                    ...
                    existing = db.query(OverheadVendor).filter(OverheadVendor.name == vendor_name).first()
                    if existing:
    ...
    with tab3:
        ...
        vendors_all = db.query(OverheadVendor).filter(OverheadVendor.is_active == True).order_by(OverheadVendor.name).all()

        if not vendors_all:
            st.info("No vendors yet. Add one first.")
        else:
            vendor_options = [v.name for v in vendors_all]
            selected_name = st.selectbox("Select vendor to edit", vendor_options, key="edit_vendor_sel")
            v = next((x for x in vendors_all if x.name == selected_name), None)

            if v:
                st.markdown("---")
                with st.form("edit_overhead_vendor"):
                    col1, col2 = st.columns(2)
                    with col1:
...
def show_expenses(db):
    ...
    with tab2:
        vendors = db.query(OverheadVendor).filter(OverheadVendor.is_active == True).order_by(OverheadVendor.name).all()
        if not vendors:
            st.warning("No vendors yet. Add vendors first.")
            return

        st.markdown("### Add New Expense")
        with st.form("add_overhead_expense", clear_on_submit=True):
            col1, col2 = st.columns(2)
            with col1:
                ...
                description = st.text_input("Description", placeholder="e.g., April electricity bill")
            with col2:
    ...
    with tab1:
        # Filters
        all_vendors = db.query(OverheadVendor).filter(OverheadVendor.is_active == True).order_by(OverheadVendor.name).all()
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            vendor_names = ["All"] + [v.name for v in all_vendors]
            filter_vendor = st.selectbox("Vendor", vendor_names, key="exp_filter_vendor")
        with col2:
            filter_category = st.selectbox("Category", ["All"] + CATEGORIES, key="exp_filter_category")
        with col3:
            current_year = date.today().year
            filter_year = st.selectbox("Year", ["All", current_year - 1, current_year, current_year + 1],
                                        index=2, key="exp_filter_year")
        with col4:
            filter_month = st.selectbox("Month", ["All"] + MONTHS,
                                         index=date.today().month, key="exp_filter_month")

        # Build query
        from sqlalchemy import extract
        from sqlalchemy.orm import joinedload
        query = db.query(OverheadExpense).options(joinedload(OverheadExpense.vendor))

        if filter_vendor != "All":
            vendor_obj = next((v for v in all_vendors if v.name == filter_vendor), None)
            if vendor_obj:
                query = query.filter(OverheadExpense.vendor_id == vendor_obj.id)
        if filter_category != "All":
            query = query.join(OverheadVendor).filter(OverheadVendor.category == filter_category)
        if filter_year != "All":
            query = query.filter(OverheadExpense.year == filter_year)
        if filter_month != "All":
            month_num = MONTHS.index(filter_month) + 1
            query = query.filter(OverheadExpense.month == month_num)

        expenses = query.order_by(OverheadExpense.expense_date.desc()).all()
        ...
        if expenses:
            ...
            filters_text = " | ".join(filters_parts) if filters_parts else "All Overhead Expenses"

            with st.expander("📄 Export to PDF"):
                if st.button("Generate PDF Report", key="gen_overhead_pdf"):
                    pdf_buffer = generate_overhead_pdf(expense_data, filters_text, total_amount, len(expenses))
                    st.download_button(
                        label="⬇️ Download PDF",
                        data=pdf_buffer,
                        file_name=f"overhead_report_{date.today().strftime('%Y%m%d')}.pdf",
    ...
    with tab3:
        ...
        all_expenses = db.query(OverheadExpense).order_by(
            OverheadExpense.expense_date.desc()
        ).limit(100).all()
        ...
        if not all_expenses:
            ...
            selected_label = st.selectbox("Select expense to edit", list(expense_options.keys()), key="edit_exp_sel")
            e = expense_options[selected_label]

            st.markdown("---")
            vendors_edit = db.query(OverheadVendor).filter(OverheadVendor.is_active == True).order_by(OverheadVendor.name).all()
            vendor_names = [v.name for v in vendors_edit]

            with st.form("edit_overhead_expense"):
                col1, col2 = st.columns(2)
                with col1:
...
def show_payments(db):
    ...
    st.markdown("---")
    st.caption("Monthly overhead payment summary — grouped by vendor")
    ...
    with col2:
        selected_month_name = st.selectbox("Month", MONTHS, index=date.today().month - 1, key="pay_month")
        selected_month = MONTHS.index(selected_month_name) + 1
    with col3:
        pt_options = ["All", "Cash", "2-Week", "Monthly"]
        filter_payment_terms = st.selectbox("Payment Terms", pt_options, key="pay_pt")
    with col4:
        filter_category = st.selectbox("Category", ["All"] + CATEGORIES, key="pay_cat")

    filter_responsible = st.selectbox("Responsible", ["All", "Diego", "Marcella", "Admin"], key="pay_resp")

    # Query
    query = db.query(OverheadExpense).options(joinedload(OverheadExpense.vendor)).filter(
        OverheadExpense.month == selected_month,
        OverheadExpense.year == selected_year
    )
    expenses = query.all()
    ...
    if expenses:
        ...
        pdf_buffer = generate_overhead_payment_pdf(
        ...
        st.download_button(
            label="📄 Download PDF",
            data=pdf_buffer,
            file_name=f"overhead_payments_{selected_month_name}_{selected_year}.pdf",
            mime="application/pdf",
            use_container_width=True,
            key="pay_pdf_dl"
        )
    else:
        st.info(f"No overhead expenses for {selected_month_name} {selected_year} matching these filters.")
